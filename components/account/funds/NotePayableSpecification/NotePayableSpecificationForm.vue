...
const payableNoteList: Ref<any[]> = ref([]);
...
const fetchData = async () => {
  await fundsStore().FETCH_ALL_NOTE("0252");

  payableNoteList.value = fundsStore().noteList;

  console.log(payableNoteList.value);

  const today = new Date();
  const year = today.getFullYear();
  const month = String(today.getMonth() + 1).padStart(2, "0"); // 월은 0부터 시작하므로 1을 더하고, 두 자리로 표현

  for (let i = 0; i < payableNoteList.value.length - 1; i++) {
    if (
      payableNoteList.value[i].maturityDate.substring(0, 7) ===
      `${year}-${month}`
    ) {
...
<template>
  <p class="text-2xl mb-6">지급어음 명세서</p>
  <div
